...
class CNN(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(32*8*8, 6) # FC(32*8*8x32)
    ...
    def forward(self, X):
        ...
        X = X.view(-1, 32*8*8) # Negative one so we can vary the batch size. # Flatten to (batch_size, 64)
...
torch.save(model.state_dict(), "modelo_final_CNN5.pth")
